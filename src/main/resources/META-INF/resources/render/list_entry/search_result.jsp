...
<div class="catalog-card-page col-lg-6 col-md-6 col-sm-6 col-xl-4 col-xs-12" style="flex: auto; max-width: none;">
	<div class="card">
		<div class="product-image-container">
			<a>
				<img class="product-image" data-href="<%= cpContentHelper.getFriendlyURL(cpCatalogEntry, themeDisplay) %>" onClick="<%= renderResponse.getNamespace() + "goToURL(event);" %>" src="<%= cpCatalogEntry.getDefaultImageFileUrl() %>" style="cursor:pointer;">
			</a>
		</div>
		...
		<div class="product-expand">
			...
			<div class="card-title" style="font-size:24px;">
				<a data-href=" <%= cpContentHelper.getFriendlyURL(cpCatalogEntry, themeDisplay) %>" onClick="<%= renderResponse.getNamespace() + "goToURL(event);" %>" style="text-decoration: underline;cursor:pointer;"><%= cpCatalogEntry.getName() %></a>
			</div>
		</div>
		...
		<div class="product-footer">
			<div class="product-actions">
				...
				<c:if test="<%= !cpCatalogEntry.isIgnoreSKUCombinations() %>">
					<a class="btn btn-block btn-outline-primary text-truncate" data-href="<%= cpContentHelper.getFriendlyURL(cpCatalogEntry, themeDisplay) %>" onClick="<%= renderResponse.getNamespace() + "goToURL(event);" %>" style="color:#1865FB;"><liferay-ui:message key="view-all-variants" /></a>
				</c:if>
			</div>
...
<aui:script>
	<portlet:namespace />goToURL = function(event) {
